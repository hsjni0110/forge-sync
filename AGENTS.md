...
코드의 동작을 추가·수정·리팩터링할 때는 각 vertical behavior마다 반드시
**RED → GREEN → REFACTOR** 순서를 지킨다. 여러 동작을 한꺼번에 구현한 뒤 테스트를 사후 추가하는
방식은 허용하지 않는다.

1. **RED**: 제품 코드를 바꾸기 전에 원하는 공개 행동을 표현하는 테스트를 먼저 추가한다. 새 기능은
   정상 경로와 관련 경계값을, 버그 수정은 보고된 문제를 재현하는 회귀 테스트를 포함한다. 해당
   테스트를 실제로 실행해 의도한 이유로 실패하는지 확인한다. 컴파일 오류나 fixture 설정 오류를
   RED 성공으로 간주하지 않는다.
2. **GREEN**: 실패한 테스트를 통과시키는 최소한의 제품 코드만 작성한다. 가장 가까운 테스트를 다시
   실행해 통과를 확인하고, 아직 검증되지 않은 미래 요구나 무관한 정리를 섞지 않는다.
3. **REFACTOR**: 테스트가 통과하는 상태에서 이름, 중복, 책임과 계층 경계를 정리한다. 동작을 새로
   추가하지 않으며, 리팩터링 도중에도 가까운 테스트를 반복 실행해 GREEN 상태를 유지한다.

한 사이클이 끝난 뒤 다음 behavior의 RED로 넘어간다. 최종 보고에는 RED에서 확인한 실패, GREEN에서
통과한 가까운 테스트, REFACTOR 후 실행한 관련 전체 검증을 구분해 남긴다. 테스트를 먼저 만들기
어려운 외부 시스템·레거시 경계라도 구현 전에 자동화 가능한 contract test, characterization test,
또는 명시적 재현 스크립트 중 하나로 RED를 만들어야 한다. 불가능하다면 임의로 건너뛰지 말고 그
이유와 대체 검증 방법을 사용자에게 알리고 결정을 요청한다.
