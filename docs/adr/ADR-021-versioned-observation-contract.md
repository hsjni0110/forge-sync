...
JSON Schema Draft 2020-12의 `ObservationEnvelope` v2.0.0을 단일 public contract로 사용한다. Edge
producer와 Factory API consumer는 동일 schema와 shared fixtures를 각 런타임의 validator로
검증한다. 정확한 schema version만 허용하고 모든 object의 미정의 field를 거절한다. 개발 초기이므로
이전 v1 계약과 fixture를 유지하는 호환 계층은 두지 않는다.

모든 Observation은 `subject.componentId`로 관찰 대상 component를 식별한다. transformation
provenance는 `sourceDataItemId`를 포함해 Devices.xml DataItem까지 추적한다. 두 identity는 source
catalog에서 가져오며 이름이나 component path로 추측하지 않는다.
...
- component별 POSITION, LOAD, TEMPERATURE와 다중 spindle 값을 서로 섞지 않고 표현할 수 있다.
- 지원 metric과 Event vocabulary는 PRD의 대표 목록으로 제한되며 나머지 DataItem은 Semantic
...
- component path만 identity로 사용: 동일한 `Rotary:C` 이름을 가진 C/C2 component를 구분하지 못한다.

## Verification

- 세 variant와 unavailable 경계 fixture를 Python과 Java validator가 모두 수락한다.
- subject, DataItem provenance, timezone, payload, unit, Replay group, schema version 또는 field 정책을 위반한
