...
| V-046 | PostgreSQL은 `2.0.0`과 `2.1.0` Observation을 같은 history 표에 보존한다 | TO_VERIFY | `V008` migration이 `schema_version` CHECK를 두 버전으로 넓히고 insert가 envelope의 버전을 그대로 저장한다. `storesUpgradedAndBaselineSchemaVersionsSideBySide` integration test를 작성했으나 이 환경에 Docker가 없어 `./scripts/verify-database`를 실행하지 못했다. | 32 | 2026-09-07 |
| V-047 | 늘어난 replay 분량에서도 broker가 메시지를 버리지 않는다 | TO_VERIFY | ADR-038의 규칙대로 queue bound를 고정 원천 관측 수 위로 유지해 60,000 → 110,000으로 올렸다(관측 101,644건). Docker가 없어 `./scripts/verify-e2e`의 dropped counter 불변과 sequence 연속성 검사를 실행하지 못했다. | 32 | 2026-09-07 |
