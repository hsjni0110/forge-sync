...
| V-035 | 3D 설비와 기능 부품은 mouse/touch/keyboard로 framing할 수 있고 검사 표현은 Twin truth를 변경하지 않는다 | VERIFIED | Box3 framing policy, 접근 가능한 camera command, explicit inspection reference, 외함 material 복원과 optional GLB capability component tests. `./scripts/verify-e2e` 실행의 실제 WebGL 시나리오에서 zoom, Main Chuck focus, enclosure transparency를 조작했고 WebGL 실패 시 2D 권위 화면도 유지했다. 전체 E2E는 별도 작업 중인 공정 도움말 변경으로 `가공의 관측 근거` locator가 사라져 마지막 시나리오에서 실패했으며 이 항목의 3D 시나리오는 통과했다. [ADR-040](./adr/ADR-040-machine-inspection-camera.md) | 22/25 | 2026-09-05 |
