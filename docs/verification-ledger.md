...
| V-020 | Inbox와 Canonical Observation은 선언된 PostgreSQL transaction 경계 안에서 effectively-once로 처리된다 | TO_VERIFY | `./scripts/verify-database`로 동일 Inbox identity 100개 동시 주입 시 acceptance/history 1건, history insert 실패 시 Inbox rollback을 실제 DB에서 확인. [ADR-025](./adr/ADR-025-postgres-ingestion-transaction.md) | 07 | - |
