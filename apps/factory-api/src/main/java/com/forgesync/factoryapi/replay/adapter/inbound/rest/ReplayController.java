...
public final class ReplayController {
  ...
  public ResponseEntity<ReplaySessionResponse> start(@RequestBody StartRequest request) {
  ...
  public ReplaySessionResponse current(@PathVariable String machineId) {
    requireIdentifier(machineId, 64, "machineId");
    return ReplaySessionResponse.from(controlReplay.current(machineId));
  }

  @PostMapping(path = "/replay-sessions/{id}/pause", produces = REPLAY_MEDIA_TYPE)
  public ReplaySessionResponse pause(@PathVariable UUID id, @RequestBody RevisionRequest request) {
    requireRevision(request.expectedRevision());
    return ReplaySessionResponse.from(controlReplay.pause(id, request.expectedRevision()));
  }

  @PostMapping(path = "/replay-sessions/{id}/resume", produces = REPLAY_MEDIA_TYPE)
  public ReplaySessionResponse resume(@PathVariable UUID id, @RequestBody RevisionRequest request) {
    requireRevision(request.expectedRevision());
    return ReplaySessionResponse.from(controlReplay.resume(id, request.expectedRevision()));
  }

  @PutMapping(path = "/replay-sessions/{id}/speed", produces = REPLAY_MEDIA_TYPE)
  public ReplaySessionResponse speed(@PathVariable UUID id, @RequestBody SpeedRequest request) {
    requireRevision(request.expectedRevision());
    return ReplaySessionResponse.from(
        controlReplay.changeSpeed(id, request.expectedRevision(), request.speedMultiplier()));
  }

  @PostMapping(path = "/replay-sessions/{id}/seek", produces = REPLAY_MEDIA_TYPE)
  public ReplaySessionResponse seek(@PathVariable UUID id, @RequestBody SeekRequest request) {
    requireRevision(request.expectedRevision());
